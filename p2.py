...
FROM (
    SELECT
        ...
        oi.order_item_name,
        all_meta.meta_data,
        flight_meta.flight_date_raw,
        CASE
            -- Format: "April 26, 2024 8:35 pm" or "April 26, 2024 20:35 pm"
            WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[A-Za-z]+ [0-9]{{1,2}}, [0-9]{{4}} [0-9]{{1,2}}:[0-9]{{2}}( )?(am|pm)$'
                THEN STR_TO_DATE(TRIM(flight_meta.flight_date_raw), '%M %e, %Y %l:%i %p')

            -- Format: "April 26, 2024 20:35"
            WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[A-Za-z]+ [0-9]{{1,2}}, [0-9]{{4}} [0-9]{{2}}:[0-9]{{2}}$'
                THEN STR_TO_DATE(TRIM(flight_meta.flight_date_raw), '%M %e, %Y %H:%i')
            -- Format: "April 26, 2024"
            WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[A-Za-z]+ [0-9]{{1,2}}, [0-9]{{4}}$'
                THEN STR_TO_DATE(TRIM(flight_meta.flight_date_raw), '%M %e, %Y')
            -- Format: "23 10 2025"

            WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[0-9]{{1,2}} [0-9]{{1,2}} [0-9]{{4}}$'
                THEN STR_TO_DATE(TRIM(flight_meta.flight_date_raw), '%d %m %Y')
            -- Format: "23 Oct 2025"
                
            WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[0-9]{1,2} [A-Za-z]{3,9} [0-9]{4}$'
                THEN STR_TO_DATE(TRIM(flight_meta.flight_date_raw), '%e %b %Y')
            -- Format: "23 October 2025"
            WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[0-9]{1,2} [A-Za-z]{4,9} [0-9]{4}$'
                THEN STR_TO_DATE(TRIM(flight_meta.flight_date_raw), '%e %M %Y')
            -- Format: "1 May 2025 15:25"
WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[0-9]{1,2} [A-Za-z]{3,9} [0-9]{4} [0-9]{1,2}:[0-9]{2}$'
    THEN STR_TO_DATE(TRIM(flight_meta.flight_date_raw), '%e %b %Y %H:%i')

-- Format: "1 May 2025 15.25"
WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[0-9]{1,2} [A-Za-z]{3,9} [0-9]{4} [0-9]{1,2}\\.[0-9]{2}$'
    THEN STR_TO_DATE(REPLACE(TRIM(flight_meta.flight_date_raw), '.', ':'), '%e %b %Y %H:%i')

-- Format: "December 3, 2025 11.30"
WHEN TRIM(flight_meta.flight_date_raw) REGEXP '^[A-Za-z]+ [0-9]{1,2}, [0-9]{4} [0-9]{1,2}\\.[0-9]{2}$'
    THEN STR_TO_DATE(REPLACE(TRIM(flight_meta.flight_date_raw), '.', ':'), '%M %e, %Y %H:%i')

            ELSE NULL
        END AS parsed_flight_date
    FROM pbDLs_posts p
    ...
    ) AS flight_meta ON oi.order_item_id = flight_meta.order_item_id
